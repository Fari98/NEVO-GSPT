...
def _run( seed, loader, resnet_v, metric, size):
    ...
    X_train,  X_test, y_train, y_test = train_test_split(X, y, p_test=1-(size/X.shape[0]), seed = seed, stratify=y)

    X_train_nn, X_val, y_train_nn, y_val = train_test_split(X_train, y_train, p_test=0.2, seed = seed, stratify=y_train)
