...
def _run( seed, loader):
    ...
    #                 epochs=1000, batch_size=32 if dataset != 'ld50' else 200)
    #
    ...
    #                      val_loss])
    #
    # train_pred = final_model.forward(X_train, y_train)
    # test_pred = final_model.forward(X_test, y_test)
    #
    # base_logger(path = f'log/final_model_trained_{day}.csv',
    #             row = [dataset,
    #                     seed,
    #                     rmse(y_train, train_pred).item(),
    #                     rmse(y_test, test_pred).item()]
    # )
